            ...
            <DialogContent className="max-w-[90vw] sm:max-w-[400px] rounded-md space-y-4">
                ...
                <div className="space-y-3">
                    ...
                    {start_end_hour && (
                        <DetailRow
                            ...
                            value={formatTime(start_end_hour)}
                        />
                    )}
                    <DetailRow
                        label="$/GPU/Day"
                        value={getValueForPricePerGPUPerDay()}
                    />
                    {total_price && (
                        <DetailRow
                            label="Total"
                            value={formatCurrency(total_price.toString())}
                            noSeparator
                        />
                    )}
                </div>
                {method === "limit" && (
                    <div className="text-sm bg-muted text-muted-foreground rounded-md p-4 text-center">
                        ...
                        <Link
                            href="/trade"
                            className="inline text-sky-600 hover:underline underline-offset-2"
                        >
                            limit order
                        </Link>
                        . You haven&apos;t actually{" "}
                        {side === "buy" ? "bought" : "sold"} anything until your
                        order gets filled.
                    </div>
                )}
